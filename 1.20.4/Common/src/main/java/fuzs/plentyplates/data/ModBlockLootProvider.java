                        ...
                        LootPool.lootPool()
                                ...
                                .add(LootItem.lootTableItem(block)
                                        .apply(CopyNbtFunction.copyData(ContextNbtProvider.BLOCK_ENTITY)
                                                .copy(PressurePlateBlockEntity.TAG_SETTINGS,
                                                        "BlockEntityTag." + PressurePlateBlockEntity.TAG_SETTINGS
                                                ))
                                        .apply(CopyNbtFunction.copyData(ContextNbtProvider.BLOCK_ENTITY)
                                                .copy(DataProvider.TAG_DATA, "BlockEntityTag." + DataProvider.TAG_DATA))
                                        .apply(CopyBlockState.copyState(block).copy(DirectionalPressurePlateBlock.LIT))
                                        .apply(CopyBlockState.copyState(block)
                                                .copy(DirectionalPressurePlateBlock.SILENT))
                                        .apply(CopyBlockState.copyState(block)
                                                .copy(DirectionalPressurePlateBlock.SHROUDED)))
                ));
    }
}
